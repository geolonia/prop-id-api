...
import { _handler } from '../src/public'
...
const main = async () => {
  ...
  for await (const rows of getQueryResult(quertExecutionId)) {
    ...
    for (const row of rows) {
      ...
      const lambdaEvent = { queryStringParameters: { q: input } } as unknown as PublicHandlerEvent
      const lambdaContext = { propId: { background: [], authentication: {} } } as unknown as PropIdContext
      const lambdaResult = await _handler(lambdaEvent, lambdaContext, x => x)
      console.log(lambdaResult)
      const { address: { ja : { prefecture, city, town, addr } } } = JSON.parse(lambdaEvent.body || '{}')
      const currentOutput = `${prefecture}${city}${town}${addr}`
