...
GeoTechnologies 社にデータがない場合でも、正しく正規化ができた住所に対しては ID を発行しています。レスポンスに含まれるジオコーディングレベルの値によって、信頼性の程度を確認できます。また、GeoTechnologies 社に存在する信頼できるデータに対しては `{ reviewed: true }` （レビュー済み）というパラメーターを返します。
この仕様は、できるだけ多くのユースケースに対応することを目的としています。住所の存在が確認できていなくても、一意の値があれば十分なケースではレビュー済みかどうかを気にせず利用できます。

レビュー済みでないデータについては存在確認の仕組みを準備中です。
...
住所の正規化が成功し、ジオコーディングができない場合には `{ reviewed: false }` ステータス付きの ID として扱われます。
