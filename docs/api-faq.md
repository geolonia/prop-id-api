...
日本の制度では土地の登記には「地番住所」、建物には「住居表示住所」が使用されます。不動産オープン ID ではジオコーダーを経由して、座標をもとにしたIDを返すというコンセプトのため、本来は同じ不動産オープン ID がアサインされるべきですが、地番住所に関しては座標の情報が不十分であり、結果的に違う ID が返されてしまいます。
